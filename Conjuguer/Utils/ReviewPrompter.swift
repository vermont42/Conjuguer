...
struct ReviewPrompter: ReviewPromptable {
  ...
  private static let defaultRequestReview: () -> Void = { (UIApplication.shared.connectedScenes.first(where: { $0.activationState == .foregroundActive }) as? UIWindowScene).map { SKStoreReviewController.requestReview(in: $0) } }
